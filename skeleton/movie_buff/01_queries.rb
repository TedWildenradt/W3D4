...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors, :castings).
  where(actors: {name: 'Harrison Ford'}).
  where.not(castings: {ord: 1}).distinct  

  # where("castings.ord != 1").distinct
  # where("actors.name = 'Harrison Ford' AND castings.ord != 1").distinct
  
  # 1) movie_id = Harrison Ford && 
  # 2) ord != 1
...
def biggest_cast
  ...
  group('movies.id').order('COUNT(movies.id) DESC').limit(3)
